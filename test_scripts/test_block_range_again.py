...
def batch_get_blocks(start_block, end_block, batch_size=50):
    ...
    for i in range(start_block, end_block + 1, batch_size):
        ...
        try:
            response = requests.post(RPC_URL, headers=HEADERS, data=json.dumps(batch)).json()
        except Exception as e:
            print(f"❌ Network or JSON error while requesting blocks {i}-{i + batch_size - 1}: {e}")
            continue
        ...
        for res in response:
            if "result" in res and res["result"] is not None:
                ...
                if start_ts <= ts <= end_ts:
                    blocks.append((int(blk["number"], 16), datetime.utcfromtimestamp(ts).strftime("%Y-%m-%d %H:%M:%S")))
            else:
