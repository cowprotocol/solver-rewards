with -- This subquery is not executable on its own.
filtered_trades as (
    select t.block_time,
        t.tx_hash,
        dex_swaps,
...
exluded_batches as (
    select tx_hash
    from filtered_trades
    where '\xf5d669627376ebd411e34b98f19c868c8aba5ada' in ("buyToken", "sellToken")
),
incoming_and_outgoing as (
    ...
    where -- We exclude settlements that have zero AMM interactions and settle several trades,
...
potential_buffer_trades as (
    ...
    group by tx_hash,
        ...
        symbol,
        token,
        block_time -- exclude 0 to prevent zero division, and exclude very small values for performance
...
valued_potential_buffered_trades as (
    ...
    from potential_buffer_trades t -- The following joins require the uniqueness of the prices per join,
        ...
        left outer join prices.usd pusd on pusd.contract_address = t.token
        and date_trunc('minute', block_time) = pusd.minute
),
internal_buffer_trader_solvers as (
    Select CONCAT('0x', ENCODE(address, 'hex'))
    from gnosis_protocol_v2."view_solvers"
    where name = 'DexCowAgg'
        or name = 'CowDexAg'
        or name = 'MIP'
        or name = 'Quasimodo'
        or name = 'QuasiModo'
...
buffer_trades as (
    Select date(a.block_time) as block_time,
        ...
        ) as matchablity_clearing_prices,
        abs(
            (a.usd_value + b.usd_value) / (abs(a.usd_value) + abs(b.usd_value))
        ) as matchability_prices_dune,
        'INTERNAL_TRADE' as transfer_type
    from valued_potential_buffered_trades a
        full outer join valued_potential_buffered_trades b on a.tx_hash = b.tx_hash
    where (
            case
                ...
                and -- We know that settlements - with at least one amm interaction - have internal buffer trades only if 
                -- the solution must come from a internal_buffer_trader_solvers solver
                (
                    a.solver_address in (
                        select *
                        from internal_buffer_trader_solvers
                    )
                    or a.dex_swaps = 0
                ) then case
                    when a.clearing_value is not null
                    and b.clearing_value is not null -- If clearing prices are use, the price of internal trades are usually pretty close to
