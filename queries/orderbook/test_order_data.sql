...
trade_hashes as (
    ...
    where settlement.block_number >= 22620415 and settlement.block_number <= 22670450
),
...
auction_prices_processed as (
    select distinct on (ap.auction_id, ap.token)
        ap.auction_id,
        ap.token,
    ...
    where s.block_number >= 22620415 and s.block_number <= 22670450
),
...
excluded_quotes as ( --noqa: PRS
    ...
    where (
        ...
        owner in ('\x687f584fd1f4a4d9eb277c03a24fe28f4b0675b7')
        and
        (
            ------------------------------------------------------- USDC/DAI ------------------------------------------------------
...
winning_quotes as (
    ...
    where
        ...
        and t.block_number >= 22620415
        and t.block_number <= 22670450
